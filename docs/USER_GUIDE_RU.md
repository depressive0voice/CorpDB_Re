...
`delete` полностью удаляет выбранную группу из конфигурации и удаляет все сохранённые requests этой группы независимо от их статуса. Поле `group` использует autocomplete существующих групп. Уже выданные этой группой Discord-роли команда не снимает автоматически; если их нужно убрать, сначала используйте `revoke` для соответствующих участников.
...
Модуль предназначен для автоматического удаления из Discord пользователей, которые слишком долго остаются с определенной trigger-role и не получают ни одну из подходящих ролей.
